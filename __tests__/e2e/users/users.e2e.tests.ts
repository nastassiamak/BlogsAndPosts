...
describe("User API", () => {
  ...
  describe("POST /users", () => {
    ...
    it("should return 400 and validation errors when input is invalid", async () => {
      ...
      const fields = response.body.errorsMessages
        .map((e: any) => e.field)
        .sort();
  ...
  describe("DELETE /users/:id", () => {
    ...
    it("should delete user by id and return 204", async () => {
      ...
      expect(getRes.body.items.some((u: any) => u.id === createdUserId)).toBe(
        false,
      );
    });

    it("should return 401 Unauthorized without authorization header", async () => {
      await request(app).delete(`${USERS_PATH}/${createdUserId}`).expect(401);
    });
